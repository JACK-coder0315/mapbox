...
map.on('load', async () => {
  ...
  trips.forEach(trip => {
    depsByMin[minutesSinceMidnight(trip.started_at)].push(trip);
    arrsByMin[minutesSinceMidnight(trip.ended_at)].push(trip);
  });
  ...
  function computeStationTraffic(stations, m = -1) {
    const depTrips = filterByMinute(depsByMin, m);
    const arrTrips = filterByMinute(arrsByMin, m);
